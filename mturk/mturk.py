...
def write_file_batch(data, filepath, delimiter=','):
    with open(filepath, 'w', newline='') as f:
        ...
        file_handler.writerow(['id', 'context', 'intents', 'intents-count', 'icons', 'link-word', 'link-word-idx', 'quantifier', 'quantifier-idx', 'category'])
        for i, item in enumerate(data):
            scenario, intents, category, link_word, link_word_idx, quantifier, quantifier_idx = item
            ...
            file_handler.writerow([i ,scenario, intent_str, len(intents), icons_str, link_word, link_word_idx, quantifier, quantifier_idx, category])


def main(input_file, output_file, limit, intents_count, link_words_count, quantifiers_count, seed=None, debug=False): 
    data = []   
    all_scenarios, all_intents, all_domains_groups, link_words, quantifiers = load_file(filepath=input_file)
    for i in range(limit):
        utterance = Utterance(all_scenarios, all_domains_groups, all_intents, link_words, quantifiers)
        scenario, intents, category, link_words, link_word_idx, quantifiers, quantifier_idx = utterance.sample(intents_count, link_words_count, quantifiers_count)
        
        data.append((scenario, intents, category, link_word, link_word_idx, quantifier, quantifier_idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--linkwordscount', type=int, default=5, help='number of link words to sample')
    parser.add_argument('--quantifierscount', type=int, default=5, help='number of quantifiers to sample')
